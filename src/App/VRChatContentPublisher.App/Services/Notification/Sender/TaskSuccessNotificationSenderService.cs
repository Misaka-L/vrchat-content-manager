...
public sealed class TaskSuccessNotificationSenderService(
    ...
    : IHostedService
{
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        {
            if (args.Status != ContentPublishTaskStatus.Completed)
                return;

            if (!appSettings.Value.SendNotificationOnTaskSucceeded)
                return;

            var contentType = MapContentTypeLabel(args.TaskService.ContentType);
            var title = string.Format(
                I18NExtension.Translate(LangKeys.Notifications_Task_Succeeded_Title_Template) ??
                "Publish {0} \"{1}\" Succeeded",
