...
namespace VRChatContentPublisher.App.Controls;

public class AsyncImageLoader
{
    private static readonly ParametrizedLogger? Logger;

    private static AppWebImageLoader ImageLoader => App.Current.AsyncImageLoader;
    ...
    private static async void OnSourceChanged(Control control, AvaloniaPropertyChangedEventArgs args)
    {
        ...
        try
        {
            if (newValue is not null)
            {
                ...
                bitmap = await ImageLoader.ProvideImageAsyncWithSize(newValue,
                    width > 0 ? width : null,
